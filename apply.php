...
// ye kari form's handle
$error = '';
$success = '';

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // em chu kormut godai apply
        ...
        if (!isset($_FILES['resume']) || $_FILES['resume']['error'] === UPLOAD_ERR_NO_FILE) {
            ...
            if (empty($error)) {
                ...
                if ($insert_stmt->execute()) {
                    ...
                    $job_query = "SELECT title, company FROM jobs WHERE id = ?";
                    $job_stmt = $conn->prepare($job_query);
                    $job_stmt->bind_param("i", $job_id);
                    $job_stmt->execute();
                    $job_result = $job_stmt->get_result();
                    ...
                    if (!$email_sent) {
                        $success .= " However, we couldn't send you a confirmation email. Please keep this page for your records.";
                    }
